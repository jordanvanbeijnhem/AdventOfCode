...
for ($i = 0; $i < count($grid); $i++) {
    for ($j = 0; $j < count($grid[0]); $j++) {
        if ($j === 0 || $j === count($grid[0]) - 1 || $i === 0 || $i === count($grid) - 1) {
            foreach ($possibleDirections as $possibleDirection) {
                $energizedTiles = [];
                emulateBeam($j, $i, $possibleDirection[0], $possibleDirection[1]);
                if (count($energizedTiles) > $mostEnergizedTiles) {
                    $mostEnergizedTiles = count($energizedTiles);
                }
            }
        }
    }
...
echo $mostEnergizedTiles . PHP_EOL;
